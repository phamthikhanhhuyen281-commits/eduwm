...
async function startServer() {
  ...
  const handleProxyStream = (req: express.Request, res: express.Response) => {
    ...
    const streamUrlWithRedirects = (currentUrl: string) => {
      try {
        ...
        const proxyReq = requestModule.get(currentUrl, { headers }, (proxyRes) => {
          // Handle redirects (very common for Google Drive and free CDNs)
          if ((proxyRes.statusCode === 301 || proxyRes.statusCode === 302 || proxyRes.statusCode === 307 || proxyRes.statusCode === 308) && proxyRes.headers.location) {
            ...
            return streamUrlWithRedirects(nextUrl);
          }
