...
export default function App() {
  ...
  // Scroll to top of section/question-navigator when active section changes
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    const container = document.getElementById('test-taking-container');
    if (container) {
  ...
  const speakingQuestions = activeExam?.questions?.speakingQuestions || (isDefaultExam ? SPEAKING_QUESTIONS : []);
  const speakingReadAloud = activeExam?.questions?.speakingReadAloud || (isDefaultExam ? { text: "The English language requires practice.", wordCount: 6 } : { text: "", wordCount: 0 });
  const grammarQuestions = activeExam?.questions?.grammar || (isDefaultExam ? GRAMMAR_QUESTIONS : []);
  const vocabularyQuestions = activeExam?.questions?.vocabulary || (isDefaultExam ? VOCABULARY_QUESTIONS : []);
            ...
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-3">
                ...
                <button
                  ...
                  className="w-full bg-white hover:bg-slate-50 text-slate-500 border border-slate-200 py-2.5 px-4 rounded-xl text-[10px] font-bold transition-all cursor-pointer"
                >
                  {t('exit_room')}
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-indigo-950/70 backdrop-blur-xs z-99 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center gap-3 text-indigo-900 border-b border-slate-100 pb-3">
              ...
              <h3 className="text-base font-bold uppercase">{t('confirm_submit_title').toUpperCase()}</h3>
            </div>

            <p className="text-xs font-medium text-slate-600 leading-relaxed">
              {t('confirm_submit_desc')}
            </p>
            ...
            <div className="bg-indigo-50/50 p-3.5 rounded-xl border border-indigo-100 text-[11px] text-indigo-950 font-semibold space-y-1">
              <div>• {lang === 'vi' ? 'Số lượng câu đã trả lời' : 'Answered questions'}: {Object.keys(answers).length} {lang === 'vi' ? 'câu' : 'questions'}</div>
              <div>• {lang === 'vi' ? 'Số lần chuyển tab cảnh báo' : 'Tab switch warnings'}: {tabSwitches} {lang === 'vi' ? 'lần' : 'times'}</div>
              <div>• {lang === 'vi' ? 'Thời gian còn lại' : 'Time remaining'}: {Math.floor(timeLeftSeconds / 60)} {lang === 'vi' ? 'phút' : 'mins'} {timeLeftSeconds % 60} {lang === 'vi' ? 'giây' : 'secs'}</div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {lang === 'vi' ? 'Hủy (Làm tiếp)' : 'Cancel (Resume)'}
              </button>
              <button
              ...
              >
                {loading 
                  ? (lang === 'vi' ? 'Đang gửi bài...' : 'Submitting...') 
                  : (lang === 'vi' ? 'Đồng ý nộp bài' : 'Confirm Submit')}
              </button>
            </div>
          </motion.div>
        </div>
