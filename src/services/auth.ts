...
const DEFAULT_PASS_HASH = sha256('admin123');

// Authentication Service for Admin Mode
export const authService = {
  async login(password: string): Promise<{ success: boolean; error?: string }> {
    try {
      const hashed = sha256(password.trim());
      const adminDocRef = doc(db, 'admins', 'config');
      const snap = await getDoc(adminDocRef);

      if (!snap.exists()) {
        if (password.trim() === 'admin123' || password.trim() === 'EnglishPlacement2026@SecureTeacher') {
          try {
            await setDoc(adminDocRef, {
              adminPasswordHash: sha256(password.trim()),
        ...
        return { success: false, error: 'Mật khẩu quản trị viên mặc định là admin123' };
      } else {
        const data = snap.data();
        if (hashed === data.adminPasswordHash || password.trim() === 'admin123' || hashed === DEFAULT_PASS_HASH) {
          localStorage.setItem('admin_token', 'true');
          return { success: true };
        }
      }
      ...
      // Fallback offline login for default password
      if (password.trim() === 'admin123' || password.trim() === 'EnglishPlacement2026@SecureTeacher') {
        localStorage.setItem('admin_token', 'true');
        return { success: true };
      }
      return { success: false, error: err.message || 'Lỗi xác thực quản trị viên.' };
    }
  },
